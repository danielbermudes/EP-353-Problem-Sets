/* This C program imposes a noise gate effect onto 'noisy.wav', and outputs it as 'noisegate.wav'.

To compile and run this program type:
clang Problem10.2.c -o Problem10.2 -lsndfile && ./Problem10.2
*/
...
#define kInputFileName "noisy.wav"
#define kOutputFileName "noisegate.wav"
#define kThreshold 0.02f //Signal Threshold (0~1. Signal Range)
...
void process(float *inBuffer, float *outBuffer, sf_count_t bufferSize){
  for(sf_count_t n = 0; n < bufferSize; n++){
    ...
    if(outBuffer[n] < kThreshold){ //if output buffer at current sample is smaller than threshold...
      outBuffer[n] = kThreshold - (outBuffer[n] * kThreshold); 
